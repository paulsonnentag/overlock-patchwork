...
export const SongConfigurator = ({
  ...
  function handleDuplicateFirstBarNotes() {
    if (
      !confirm(
        "WARNING: Are you sure you want to overwrite all other bars with the first bar notes?"
      )
    ) {
      return;
    }
  ...
  function handleDuplicateFirstBarDrums() {
    if (
      !confirm(
        "WARNING: Are you sure you want to overwrite all other bars with the first bar drums?"
      )
    ) {
      return;
    }
  ...
  function handleDuplicateFirstBar() {
    if (
      !confirm(
        "WARNING: Are you sure you want to overwrite all other bars with the first bar?"
      )
    ) {
      return;
    }
  ...
  return (
    <div>
      <button
        className="button config-button float-left"
        onClick={flipMonophonic}
      >
        {text}
      </button>
      ...
      <div className="horizontal-block"></div>
      <div className="explanation float-left">
        The options below are destructive and can interfere with collaboration!
      </div>
      <div className="clear"></div>
      <div className="horizontal-block"></div>
      <button
        className="button duplicate-button float-left"
        onClick={handleDuplicateFirstBarNotes}
      >
        Paste Notes
      </button>
      <div className="horizontal-block"></div>
      <button
        className="button duplicate-button float-left"
        onClick={handleDuplicateFirstBarDrums}
      >
        Paste Drums
      </button>
      <div className="horizontal-block"></div>
      <button
        className="button duplicate-button float-left"
        onClick={handleDuplicateFirstBar}
      >
        Paste All
      </button>
      <div className="horizontal-block"></div>
      <button
        className="button clear-button float-left"
      ...
      <div className="float-left">
        <label>
          ...
          <select onChange={changeStepsPerBar} value={config.stepsPerBar}>
            <option value={8} key="8steps">
              8
            </option>
            <option value={16} key="16steps">
              16
            </option>
          </select>
        </label>
      ...
      <div className="solid-outline">
        <label>
          ...
          <input
            name="fetchUrl"
            className="solid-outline"
            style={{ marginLeft: "20px" }}
            type="string"
            onChange={handleSetFetchUrl}
          ></input>
          <button
            className="button fetch-button"
            style={{ marginLeft: "20px" }}
            onClick={handleFetchOverridingInstrument}
          >
            Fetch
          </button>
        </label>

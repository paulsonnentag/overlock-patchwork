...
import {
  ...
  Note,
  Step,
} from "../music/instrument-scheduler";
...
export function updateStepGridFromToggles(
  toggles: Toggle[][],
  drumToggles: Toggle[][],
  stepGrid: Step[],
  config: SongConfig
...
function updateStepFromDrumToggle(
  isToggled: boolean,
  x: number,
  y: number,
  stepGrid: Step[],
...
export const Player = ({
  ...
  function handleVolumeChange(e: any) {
    ...
    if (globalInstrumentSchedulers.length > 0) {
      globalInstrumentSchedulers[0].updateInstrumentVolumeModifier(
        e.target.value
      );
  ...
  let playClasses = classnames("button", "float-left", {
  ...
  let directionClasses = classnames("button", "direction-button", "float-left");
  ...
  return (
    <div>
      <button className={playClasses} onClick={handleTogglePlay}>
        {text}
      </button>
      <div className="horizontal-block"></div>
      <button className={directionClasses} onClick={toggleDirection}>
        {direction}
      </button>
      <div className="horizontal-block"></div>
      <div className="float-left">
        ...
        <input
          onChange={handleDrumVolumeChange}
          type="range"
          min="0.0"
          max="1.0"
          step="0.01"
